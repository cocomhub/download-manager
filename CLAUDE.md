...
### CancelObject 与 resolve worker / download 时序竞争（已修复 2026-06-19, 2026-06-23）

**resolve worker 路径（已修复 2026-06-19）**：`processResolve` resolve 成功后无条件 `UpdateStatus(obj, StatusPending)`，无视对象是否已被 `CancelObject()` 取消。
**修复**：在 `processResolve` 中覆盖 pending 前，从 storage 重新读取对象状态，若为 `cancelled` 则保留取消状态（`SetStatusUnlessCancelled`）。

**download 路径（已修复 2026-06-23）**：`download()` 在 `dl.Download()` 完成后用本地 stale pointer 写入 `completed` 或 `failed`，即使 `CancelObject` 已在另一 goroutine 中修改 storage 为 `cancelled`。
**修复**：新增 `isCancelled()` 辅助函数从 storage 重读最新状态，在成功路径标记 `completed` 前和失败路径检查 cancelled 时使用。

**核心模式**：任何时候从一个 goroutine 修改的对象状态在另一个 goroutine 中被写入时，不能信任本地指针——必须从 storage 重新读取。
