...
func TestRunMediaBackfill_EnqueuesCoverOncePerBook(t *testing.T) {
	...
	cover := filepath.Join(t.TempDir(), "book", "cover.jpg")
	for _, u := range []string{"u1", "u2", "u3"} {
		obj := &model.DownloadObject{URL: u, TaskID: "t1", Status: "pending", Extra: map[string]any{}}
		if err := store.Update(obj); err != nil {
			t.Fatalf("seed %s: %v", u, err)
		}
	}
	m.tasks.Store("t1", &sharedCoverTask{
		mockTask:  &mockTask{id: "t1", typ: "mxs"},
		store:     store,
		coverPath: cover,
	})

	NewStandardizationService(m).runMediaBackfill(t.Context())

	// 三个章节共享同一封面 → 回填只入队 1 次。
	if len(m.soQueue) != 1 {
		t.Errorf("soQueue len = %d, want 1 (one cover per book)", len(m.soQueue))
	}
}
