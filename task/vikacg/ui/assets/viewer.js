    ...
    TaskUI.register('vikacg', {
      ...
      renderViewer: function (h, obj, onClose) {
        ...
        var Wrapper = {
          ...
          methods: {
            prev: function () {
              this.currentIdx = (this.currentIdx - 1 + images.length) % images.length
            },
            next: function () {
              ...
              h('div', { class: 'bg-white rounded-lg shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col' }, [
                ...
                h('div', { class: 'flex-1 overflow-y-auto' }, [
                  ...
                  h('div', { class: 'relative bg-black flex items-center justify-center', style: { minHeight: '300px' } }, [
                    ...
                    images.length > 1 ? [
                      h('button', {
                        class: 'absolute left-2 top-1/2 transform -translate-y-1/2 bg-white/80 rounded-full w-9 h-9 flex items-center justify-center text-gray-700 hover:bg-white shadow',
                        on: { click: function (e) { e.stopPropagation(); self.prev() } }
                      }, [h('i', { class: 'fas fa-chevron-left' })]),
                      h('button', {
                        class: 'absolute right-2 top-1/2 transform -translate-y-1/2 bg-white/80 rounded-full w-9 h-9 flex items-center justify-center text-gray-700 hover:bg-white shadow',
                        on: { click: function (e) { e.stopPropagation(); self.next() } }
                  ...
                  images.length > 1 ? h('div', { class: 'flex gap-2 p-2 overflow-x-auto bg-gray-100' }, images.map(function (src, idx) {
                    return h('img', {
                      attrs: { src: src, alt: 'Thumb ' + (idx + 1) },
                      class: 'w-16 h-16 object-cover rounded cursor-pointer',
                      style: {
                        border: '2px solid ' + (idx === currentIdx ? '#3b82f6' : '#e5e7eb'),
                        boxShadow: idx === currentIdx ? '0 0 0 2px #3b82f6' : 'none'
                      },
                      on: { click: function () { self.goTo(idx) } }
                    })
                  })) : null,
                  // Content area
                  h('div', { class: 'p-4' }, [
                    html ? h('div', { class: 'text-sm text-gray-700 leading-relaxed' }, html.replace(/<[^>]+>/g, ' ')) :
                      excerpt ? h('div', { class: 'text-sm text-gray-700 whitespace-pre-line leading-relaxed' }, excerpt) : null,
                    // Tags
                    tags.length ? h('div', { class: 'flex flex-wrap gap-1 mb-3 mt-3' }, tags.map(function (tag) {
                      return h('span', { class: 'text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded' }, '#' + tag)
                    })) : null,
                    // Links
                    links.length ? [
                      h('p', { class: 'text-xs font-semibold text-gray-500 mb-1' }, '相关链接'),
                      h('ul', { class: 'space-y-1' }, links.map(function (l) {
                        return h('li', [
                          h('a', {
                ...
                h('div', { class: 'p-3 border-t bg-gray-50 flex justify-between items-center' }, [
                  h('div', { class: 'flex gap-2' }, [
                    pageUrl ? h('button', {
                      ...
                      on: { click: function () { window.open(pageUrl, '_blank', 'noopener,noreferrer') } }
                    }, '打开原页面') : null,
                    pageUrl ? h('button', {
                  ...
                  h('button', {
                    class: 'px-3 py-1.5 rounded bg-white border text-sm hover:bg-gray-100',
                    on: { click: function (e) { e.stopPropagation(); if (onClose) onClose() } }
                  }, '关闭'),
                ]),
              ])
            ])
