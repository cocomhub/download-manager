...
- [ ] 引入 RootFS 路径沙盒
  ...
  - [x] downloader/native 初始化时派生 root_dir（默认 <server.work_dir>/downloads）并传入
- [ ] 替换文件系统操作
  ...
  - [x] 将代理缓存目录迁移到 root 内的 cache_dir（不再使用 os.TempDir）
- [ ] 外部化硬编码参数
  - [x] 将默认 UA/headers、HTTP 超时与连接池、探测超时与 TTL、缓冲区大小、权限掩码、进度阈值参数化
  - [x] 为 ffmpeg 提供 extra_args（以当前默认参数为默认值），并外部化 HLS 相关开关
  - [x] 删除环境专属绝对路径逻辑，改为可选配置（在 root 内）
- [ ] 兼容与迁移
  - [x] 继续支持既有 Option（WithLoggerDir、WithFFmpegPath 等），内部映射到新配置
  - [x] 若未配置 root_dir，则自动采用 <server.work_dir>/downloads
  - [ ] 文档化迁移注意事项
- [ ] 测试与验证
