...
### P6-3 njavtv 站点任务（番号库 + 代理抓取）
- 用户需求（2026-09-25）：新增 njavtv.com 任务（经 127.0.0.1:1080 代理访问）
- 支持按番号、厂商、标签、类型爬取；支持番号提取
- 番号库机制：pkg/idregistry（JSON 持久化），含 tktube 已抓取番号构建
- 去重：已知番号跳过；未知番号不去重但打 metadata.special_review=true（人工复核标记）
- 站点实证：列表页 /dm265/en、详情页 /dm<id>/en/<dvd_id>、封面/预览 fourhoi.com（可下载）
